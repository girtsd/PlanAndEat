...
// 2.Perform database query.
    ...
    <?php echo form_errors($errors);?>    
        <h2>Create Weight</h2>
        <form action="weight_all_update.php" method="post" >
        
 <?php      echo "<input type='text' name=".$row["Weight"]." value=".$row["Weight"]."> ";  
 
            while ($row = mysqli_fetch_assoc($rs_result)) { 
            echo "<p> <label>".$row["UnitName"]."</label>";
            echo "<input type='text' name=".$row["Weight"]." value=".$row["Weight"]."> "; 
            };                       
 ?>       
