...
//            order by m.Date, m.category_id LIMIT {$start_from}, 20";
        ...
        if ($rs_result) {
            echo $mdate;      
            echo "<table>";
            echo "<tr><th>ProductName</th><th>Amount</th><th>Unit</th></tr>";
        ...
        ?>
        <?php 
            $sql_cnt = "select @row := @row + 1 as row, t1.ProductName from (select p.ProductName ProductName, sum(c.Amount) Amount, c.Unit Unit
            from Menu m, Components c, Products p
            where m.Date between '$mdate' and date_add('$mdate', INTERVAL 7 DAY)
            and c.Recipe_id = m.Recipe_id
            and p.Product_id = c.Product_id
            GROUP BY p.ProductName, c.Unit) t1, (SELECT @row := 0) t2";
